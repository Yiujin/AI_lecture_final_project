...
if __name__ == '__main__':
    ...
    parser.add_argument("--epochs", required=False, default=1000, help="epochs : default=100")
    parser.add_argument("--mini_batch_size", required=False, default=100, help="mini_batch_size : default=100")
    parser.add_argument("--learning_rate", required=False, default=0.001, help="learning_rate : default=0.01")
    args = parser.parse_args()

    # 데이터셋 탑재
    (x_train, t_train), (x_test, t_test) = load_AReM(one_hot_label=True) # one-hot true

    #feature scailing을 위한 전체 학습데이터 평균, 표준편차 구함
    xtrain_mean = np.mean(x_train, axis = 0)
    xtrain_std = np.std(x_train, axis = 0)
    

    # 모델 초기화
    network = Model(xtrain_mean, xtrain_std, lr = 0.001)
    ...
    # 손실함수 값 plot 그리기
    x = np.arange(trainer.current_iter)
